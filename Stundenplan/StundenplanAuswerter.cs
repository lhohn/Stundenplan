...
namespace Stundenplan
{
    public class StundenplanAuswerter
    {
        ...
        {
            //Settings();
            path = @"C:\Users\Luis Laptop\source\repos\Stundenplan\Stundenplan\stundenplan.xml";
            string exit = "";
            while (exit != "exit")
            {
    ...
    private static void AuswaertenByDay()
        {
            ...
            foreach (XElement item in xml.Root.Elements(doW.ToString()))
            {
                foreach (XElement fach in item.Nodes())
                {
                    ...
                    TimeSpan current = TimeSpan.Parse(string.Concat(DateTime.Now.Hour, ":", DateTime.Now.Minute)); //TimeSpan.Parse("10:45");//
